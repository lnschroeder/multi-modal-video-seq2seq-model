...
def ConvKxKBn(filters, use_batchnorm, kernel_size, name=None):
    kwargs = get_submodules()
...
def build_linknet(
    ...
    encoding = backbone.output
    kernel_size = (encoding.shape[1], encoding.shape[2])

    encoding = ConvKxKBn(dim_encoding, use_batchnorm, kernel_size, name='last_conv2d')(encoding)

    encoding = backend.squeeze(encoding, axis=1)
    encoding = backend.squeeze(encoding, axis=1)
    ...
    if isinstance(backbone.layers[-1], layers.MaxPooling2D):
        encoding = Conv3x3BnReLU(512, use_batchnorm, name='center_block1')(encoding)
        encoding = Conv3x3BnReLU(512, use_batchnorm, name='center_block2')(encoding)

    # decoder
    decoder_input = [
        layers.Input(shape=backend.int_shape(encoding)[1:]),
        [
            layers.Input(shape=backend.int_shape(backbone.get_layer(name=i).output)[1:]) if isinstance(i, str)
            else layers.Input(shape=backend.int_shape(backbone.get_layer(index=i).output)[1:])
    ...
    x = layers.Conv2DTranspose(10, kernel_size, padding='valid', use_bias=not use_batchnorm)(x)

    for i in range(n_upsample_blocks):

        if i < len(skips):
            skip = decoder_input[1][i]
        else:
            skip = None
    ...
    decoding = layers.Activation(activation, name=activation)(x)

    # create keras model instance
    encoder = models.Model(input_, [encoding, skips])
    decoder = models.Model(decoder_input, decoding)
    autoencoder = models.Model(input_, decoder(encoder.output))
...
def Linknet(
    ...
    encoder, decoder, autoencoder = build_linknet(
